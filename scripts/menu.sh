...
create_profile() {
    ...
    # Create profile with appropriate flags
    echo -e "${YELLOW}Creating profile with $ip_type address...${NC}"
    if [ "$ip_type" == "ipv6" ]; then
        if $MANAGER_PATH profile create --id "$profile_id" --name "$profile_name" $use_ipv6_flag "$ip_address" --port "$port" --sni "$sni" $masquerade_flag; then
